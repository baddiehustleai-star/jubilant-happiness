...
export const BrandLink = ({
  ...
  const baseClasses =
    'transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:ring-offset-2 rounded';
...
export const BrandQuote = ({ children, author, role, variant = 'default', className = '' }) => {
  const variantClasses = {
  ...
  return (
    <blockquote className={`pl-6 py-4 ${variantClasses[variant]} ${className}`}>
      <BrandText className="italic text-lg mb-3">"{children}"</BrandText>
      {author && (
        <footer>
...
export const BrandList = ({
  ...
  const variantClasses = {
    ...
    simple: '',
    checkmark:
      '[&>li]:flex [&>li]:items-start [&>li]:space-x-3 [&>li::before]:content-["✓"] [&>li::before]:text-green-500 [&>li::before]:font-bold [&>li::before]:mr-2',
    bullet:
      '[&>li]:flex [&>li]:items-start [&>li]:space-x-3 [&>li::before]:content-["•"] [&>li::before]:text-rose-500 [&>li::before]:font-bold [&>li::before]:mr-2',
  };

  return (
    <ul
      className={`${baseClasses} ${spacingClasses[spacing]} ${variantClasses[variant]} ${className}`}
    >
      {children}
    </ul>
  );
