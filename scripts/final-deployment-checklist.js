  ...
  try {
    const response = await fetch(`${CLOUD_RUN_URL}/api/seo/refresh?limit=1`, {
      ...
      headers: {
        ...
        "x-cron-secret": process.env.CRON_SECRET || process.env.SHARED_WEBHOOK_SECRET || "test-secret"
      }
    });
    ...
    if (response.ok) {
      const data = await response.json();
      if (data.success) {
        logCheck("SEO Refresh", "pass", `Refreshed ${data.refreshed} products (examined ${data.examined})`);
        return true;
      } else {
      ...
      logCheck("SEO Refresh", "warn", "Endpoint requires valid CRON_SECRET (configure in production)");
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      if (html.includes("<!DOCTYPE html>") && html.includes("<title>")) {
        ...
        logCheck("Product Pages", "warn", "Product page route exists but HTML structure unexpected");
        return false;
      }
    } else if (response.status === 404) {
      logCheck("Product Pages", "pass", "Product page route exists (404 for test product is expected)");
      return true;
    } else {
      logCheck("Product Pages", "fail", `Product page route returned unexpected status ${response.status}`);
      return false;
    }
  } catch (error) {
